...
class HomeController extends Controller
{
  ...
  {
    //CV
    $data['sliders'] = Slider::published()->orderBy('updated_at', 'desc')->get();
    // dd($data['sliders']);

    $data['travelwithuss'] = Whywithus::published()->orderBy('updated_at', 'desc')->take(6)->get();
    $data['travelersreview'] = Travelersreview::published()->orderBy('updated_at', 'desc')->take(3)->get();
    $data['blogs'] = Blog::published()->orderBy('updated_at', 'desc')->latest()->take(2)->get();
    $data['associated'] = Associate::published()->orderBy('updated_at', 'desc')->get();

    //package
    $data['upcomingTrek'] = Package::published()->where('upcoming_trips', 1)->orderBy('updated_at', 'desc')->limit(3)->get();
    $data['bestSells'] = Package::published()->where('best_sells', 1)->orderBy('updated_at', 'desc')->limit(12)->get();
    $data['recommandedPackages'] = Package::published()->where('recommanded_package', 1)->orderBy('updated_at', 'desc')->limit(9)->get();

    $data['bestSells'] = Package::published()->where('best_sells', 1)->orderBy('updated_at', 'desc')->limit(6)->get();
    $data['adventureRoadTrip'] = Package::published()->where('adventure_road_trip', 1)->orderBy('updated_at', 'desc')->limit(3)->get();
    $data['adventureRoadTrip'] = Package::published()->where('adventure_road_trip', 1)->orderBy('updated_at', 'desc')->limit(3)->get();
    $data['destinations'] = Destination::published()->orderBy('created_at', 'desc')->take(6)->get();
    $data['travelStyles'] = Travelstyle::published()->orderBy('created_at', 'asc')->take(6)->get();

    //end CV

    $data['trip_of_the_month'] = Package::published()->where('trip_of_the_month', 1)->orderBy('updated_at', 'desc')->first();
    $data['homepagevideo'] = \App\Models\Video::published()->homepage()->orderBy('updated_at', 'desc')->first();
    // FIND OUR POPULAR TOOLS
    $trekkingPackage = Package::published()->orderBy('updated_at', 'desc')->take(2)->get();
    $toursPackage = TourPackage::published()->populartours()->orderBy('updated_at', 'desc')->take(2)->get();
    $adventurePackage = AdventurePackage::published()->populartours()->orderBy('updated_at', 'desc')->take(2)->get();
    $helitourPackage = HelitourPackage::published()->populartours()->orderBy('updated_at', 'desc')->take(2)->get();
    $naturePackage = NaturePackage::published()->populartours()->orderBy('updated_at', 'desc')->take(2)->get();
    $popularPackage = $trekkingPackage->concat($toursPackage)->concat($adventurePackage)->concat($helitourPackage)->concat($naturePackage);
    //Search Package
    $trekkingPackageSearch = Package::published()->get();
    $toursPackageSearch = TourPackage::published()->get();
    $adventurePackageSearch = AdventurePackage::published()->get();
    $helitourPackageSearch = HelitourPackage::published()->get();
    $naturePackageSearch = NaturePackage::published()->get();
    $packageSearch = $trekkingPackageSearch->concat($toursPackageSearch)->concat($adventurePackageSearch)->concat($helitourPackageSearch)->concat($naturePackageSearch);
    // dd($packageSearch);

    // UNIQUE AND EXCLUSIVE ADVENTURES
    $aboutus = Page::published()->where('slug', 'about-north-nepal-travel-treks')->first();

    $data['activity'] = Activity::published()->orderBy('created_at', 'desc')->get();
    $data['destination'] = Destination::published()->orderBy('updated_at', 'desc')->take(4)->get();
    // SEARCH FOR TOUR durations part
    $data['package'] = Package::published()->orderBy('created_at', 'desc')->take(4)->get();

    // UPCOMING TREKS
    $data['upcomig_treks'] = Package::published()->orderBy('created_at', 'desc')->first();
    // dd($data['upcomig_treks']);



    return view('front.index', $data, compact('aboutus', 'packageSearch', 'popularPackage'));
  }
  ...
  {
      $og['title'] = 'Team of Adventure Magic Treks, Our Team,Best Trekking company in Nepal';
    $details = Team::published()->orderBy('updated_at', 'desc')->get();
  ...
  {
    //Pravacy policy, term and condition CV
    if ($slug == 'privacy-policy') {
      try {
        ...
        return view('front.pages.privacyPolicy', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'travel-terms-bookings') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.travelsTerm', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'nepal-visa') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.nepalVisa', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'accommodations') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.accommodation', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'best-time-to-travel') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.bestTimeToTravel', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'travel-safety-tips') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.travelSafetyTips', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'packing-list') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.packingList', compact('detail','og'));
        die;
        } catch (\Exception $ae) {
          abort('404');
        }
    ...
    if ($slug == 'altitude-sickness-treatment') {
      try {
        $detail = \App\Models\Page::published()->where('slug', $slug)->first();
        $og['title'] = $detail->page_title ?? $detail->title;
        $og['description'] = $detail->meta_description;
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.altitudeSicknessTreatment', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'travel-insurance-and-vaccinations') {
      try {
        $detail = \App\Models\Page::published()->where('slug', $slug)->first();
        $og['title'] = $detail->page_title ?? $detail->title;
        $og['description'] = $detail->meta_description;
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.travelInsurance', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'vaccination') {
      try {
        ...
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.vaccination', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
    ...
    if ($slug == 'about-north-nepal-travel-treks') {
      try {
        $detail = \App\Models\Page::published()->where('slug', $slug)->first();
        $og['title'] = $detail->page_title ?? $detail->title;
        $og['description'] = $detail->meta_description;
        $og['keywords'] = $detail->keywords;
        // $relatedDestination = Destination::published()->inRandomOrder()->take(3)->get();
        return view('front.pages.aboutus', compact('detail','og'));
        die;
      } catch (\Exception $ae) {
        abort('404');
      }
  ...
  {
    $details = Blog::published()->whereSlug($slug)->first();
    $og['title'] = $details->page_title;
    $og['description'] = $details->meta_description;
    $og['keywords'] = $details->keyword;
    $relatedBlogs = Blog::published()->where('id', '!=', $details->id)->inRandomOrder()->orderBy('updated_at', 'desc')->take(5)->get();
    return view('front.blog.details', compact('details', 'relatedBlogs', 'og'));
  }
  ...
  {
    //  try {
    $packages = Package::published()->whereSlug($package_slug)->first();
    // dd($packages);
    $og['title'] = $packages->meta_title ?? $packages->package_name;
    $og['description'] = $packages->meta_description;
    $og['keywords'] = $packages->keyword;

    //  if (is_null(@$packages)) {
    //    abort('404');
    //  } else {
    $similarTrekkingPackage = Destinationtype::findorfail($packages->destinationtype_id)->packages()->where('id', '!=', $packages->id)->published()->take(4)->get();
    // dd($similarTrekkingPackage);
    //if similar package is null then random will be passed to view
    if (@$similarTrekkingPackage->count() == 0) {
      $similarTrekkingPackage = Package::published()->inRandomOrder()->take(4)->get();
      // dd($similarTrekkingPackage);
    }
